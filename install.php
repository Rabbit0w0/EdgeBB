...
function _p($adapter) {
    switch ($adapter) {
        case 'Mysql':
            return Edge_Db_Adapter_Mysql::isAvailable();
        case 'Mysqli':
            return Edge_Db_Adapter_Mysqli::isAvailable();
        case 'Pdo_Mysql':
            return Edge_Db_Adapter_Pdo_Mysql::isAvailable();
        case 'SQLite':
            return Edge_Db_Adapter_SQLite::isAvailable();
        case 'Pdo_SQLite':
            return Edge_Db_Adapter_Pdo_SQLite::isAvailable();
        case 'Pgsql':
            return Edge_Db_Adapter_Pgsql::isAvailable();
        case 'Pdo_Pgsql':
            return Edge_Db_Adapter_Pdo_Pgsql::isAvailable();
        default:
            return false;
    }
}
 ...
 */
function _u() {
    $url = Edge_Request::getUrlPrefix() . $_SERVER['REQUEST_URI'];
...
/** 获取语言 */
$lang = _r('lang', Edge_Cookie::get('__edge_lang'));
...
if ('zh_CN' != $lang) {
    ...
    Edge_I18n::setLang($dir . '/' . $lang . '.mo');
}

Edge_Cookie::set('__edge_lang', $lang);
?><!DOCTYPE HTML>
...
<div class="container">
    <div class="row">
        <div class="col-mb-12 col-tb-8 col-tb-offset-2">
            <div class="column-14 start-06 edge-install">
            <?php if (isset($_GET['finish'])) : ?>
                <?php if (!isset($db)) : ?>
                <h1 class="edge-install-title"><?php _e('安装失败!'); ?></h1>
                <div class="edge-install-body">
                ...
                <?php elseif (!Edge_Cookie::get('__edge_config')): ?>
                    <h1 class="edge-install-title"><?php _e('没有安装!'); ?></h1>
                    <div class="edge-install-body">
                        <form method="post" action="?config" name="config">
                ...
                <?php else : ?>
                    ...
                    ?>
                <h1 class="edge-install-title"><?php _e('安装成功!'); ?></h1>
                <div class="edge-install-body">
                    ...
                    <p><?php _e('希望您能尽情享用 EdgeBB 带来的乐趣!'); ?></p>
                </div>
                <?php endif;?>
            <?php elseif (isset($_GET['start'])): ?>
                ...
                <?php else : ?>
            <?php
                                    $config = unserialize(base64_decode(Edge_Cookie::get('__edge_config')));
                                    ...
                                    try {
                                        ...
                                        $scripts = str_replace('edge_', $config['prefix'], $scripts);
                                        ...
                                        foreach ($scripts as $script) {
                                            $script = trim($script);
                                            if ($script) {
                                                $installDb->query($script, Edge_Db::WRITE);
                                            }
                                        }
                                        ...
                                        $installDb->query($installDb->insert('table.options')->rows(array('name' => 'keywords', 'user' => 0, 'value' => 'edge,php,blog')));
                                        ...
                                        $installDb->query($installDb->insert('table.options')->rows(array('name' => 'secret', 'user' => 0, 'value' => Edge_Common::randString(32, true))));
                                        ...
                                        /** 初始内容 */
                                        $installDb->query($installDb->insert('table.contents')->rows(array('title' => _t('欢迎使用 Edge'), 'slug' => 'start', 'created' => Edge_Date::time(), 'modified' => Edge_Date::time(),
                                        'text' => '<!--markdown-->' . _t('如果您看到这篇文章,表示您的 EdgeBB 已经安装成功.'), 'authorId' => 1, 'type' => 'post', 'status' => 'publish', 'commentsNum' => 1, 'allowComment' => 1,
                                        'allowPing' => 1, 'allowFeed' => 1, 'parent' => 0)));

                                        $installDb->query($installDb->insert('table.contents')->rows(array('title' => _t('关于'), 'slug' => 'start-page', 'created' => Edge_Date::time(), 'modified' => Edge_Date::time(),
                                        'text' => '<!--markdown-->' . _t('本页面由 EdgeBB 创建, 然而这只是个测试页面.'), 'authorId' => 1, 'order' => 0, 'type' => 'page', 'status' => 'publish', 'commentsNum' => 0, 'allowComment' => 1,
                                        'allowPing' => 1, 'allowFeed' => 1, 'parent' => 0)));

                                        /** 初始评论 */
                                        $installDb->query($installDb->insert('table.comments')->rows(array('cid' => 1, 'created' => Edge_Date::time(), 'author' => 'Edge', 'ownerId' => 1, 'url' => 'http://mcedge.ink',
                                        'ip' => '127.0.0.1', 'agent' => $options->generator, 'text' => '欢迎加入 EdgeBB', 'type' => 'comment', 'status' => 'approved', 'parent' => 0)));
                                        ...
                                        'url' => 'http://www.mcedge.ink', 'screenName' => $config['userName'], 'group' => 'administrator', 'created' => Edge_Date::time())));
...
<?php
            ...
            ?>
                <form method="post" action="?config" name="config">
                    <h1 class="edge-install-title"><?php _e('确认您的配置'); ?></h1>
                    <div class="edge-install-body">
                        <h2><?php _e('数据库配置'); ?></h2>
                        <?php
                            if ('config' == _r('action')) {
                                ...
                                if (!isset($config) && $success && !_r('created')) {
                                    $installDb = new Edge_Db($adapter, _r('dbPrefix'));
                                    $installDb->addServer($dbConfig, Edge_Db::READ | Edge_Db::WRITE);


                                    /** 检测数据库配置 */
                                    try {
                                        $installDb->query('SELECT 1=1');
                                    } catch (Edge_Db_Adapter_Exception $e) {
                                        $success = false;
                                ...
                                if($success) {
                                    ...
                                    Edge_Cookie::set('__edge_config', base64_encode(serialize(array_merge(array(
                                        'prefix'    =>  _r('dbPrefix'),
                                    ...
                                    $lines[] = "
/** 定义数据库参数 */
\$db = new Edge_Db('{$adapter}', '" . _r('dbPrefix') . "');
\$db->addServer(" . (empty($config) ? var_export($dbConfig, true) : $config) . ", Edge_Db::READ | Edge_Db::WRITE);
Edge_Db::set(\$db);
";
                                    $contents = implode('', $lines);
                                    if (!Edge_Common::isAppEngine()) {
...
<p><button name="created" value="1" type="submit" class="btn primary"><?php _e('创建完毕, 继续安装 &raquo;'); ?></button></p></div>
                                ...
                                // 安装不成功删除配置文件
                                if(!$success && file_exists(__EDGE_ROOT_DIR__ . '/config.inc.php')) {
                                    @unlink(__EDGE_ROOT_DIR__ . '/config.inc.php');
                                }
                            }
                        ?>
                        <ul class="edge-option">
                            <li>
                            <label for="dbAdapter" class="edge-label"><?php _e('数据库适配器'); ?></label>
                            ...
                            <li>
                            <label class="edge-label" for="dbPrefix"><?php _e('数据库前缀'); ?></label>
                            <input type="text" class="text" name="dbPrefix" id="dbPrefix" value="<?php _v('dbPrefix', 'edge_'); ?>" />
                            <p class="description"><?php _e('默认前缀是 "edge_"'); ?></p>
                            </li>
                        </ul>
                        ...
                        <ul class="edge-option">
                            <li>
                            <label class="edge-label" for="userUrl"><?php _e('网站地址'); ?></label>
                            ...
                            <li>
                            <label class="edge-label" for="userMail"><?php _e('邮件地址'); ?></label>
            ...
            <?php  else: ?>
                ...
                <h1 class="edge-install-title"><?php _e('欢迎使用 Edge'); ?></h1>
                <div class="edge-install-body">
                <h2><?php _e('安装说明'); ?></h2>
