 ...
 * @license MIT License
 */
class Edge_Widget_Helper_Form extends Edge_Widget_Helper_Layout
{
    /** 表单post方法 */
     ...
     *
     * @access public
     * @param Edge_Widget_Helper_Form_Element $input 输入元素
     * @return Edge_Widget_Helper_Form
     */
    public function addInput(Edge_Widget_Helper_Form_Element $input)
    {
    ...
    public function validate()
    {
        ...
        if ($error) {
            /** 利用session记录错误 */
            Edge_Cookie::set('__edge_form_message_' . $id, Json::encode($error));

            /** 利用session记录表单值 */
            Edge_Cookie::set('__edge_form_record_' . $id, Json::encode($formData));
        }
    ...
    {
        $id = md5(implode('"', array_keys($this->_inputs)));
        $record = Edge_Cookie::get('__edge_form_record_' . $id);
        $message = Edge_Cookie::get('__edge_form_message_' . $id);
        ...
        if (!empty($record)) {
            ...
            Edge_Cookie::delete('__edge_form_record_' . $id);
        }

        parent::render();
        Edge_Cookie::delete('__edge_form_message_' . $id);
    }
}
