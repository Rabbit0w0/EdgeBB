 ...
 * @license MIT License
 */
abstract class Edge_Widget_Helper_Form_Element extends Edge_Widget_Helper_Layout
{
    /**
     * 表单描述
     *
    ...
    {
        /** 创建html元素,并设置class */
        parent::__construct('ul', array('class' => 'edge-option', 'id' => 'edge-option-item-' . $name . '-' . self::$uniqueId));
        $this->name = $name;
        self::$uniqueId ++;
    ...
    public function label($value)
    {
        /** 创建标题元素 */
        if (empty($this->label)) {
            $this->label = new Edge_Widget_Helper_Layout('label', array('class' => 'edge-label'));
     ...
     *
     * @access public
     * @param Edge_Widget_Helper_Layout $item 表单元素
     * @return $this
     */
    public function container(Edge_Widget_Helper_Layout $item)
    {
        /** 创建表单容器 */
        if (empty($this->container)) {
            $this->container = new Edge_Widget_Helper_Layout('li');
    ...
    public function message($message)
    {
        if (empty($this->message)) {
            $this->message =  new Edge_Widget_Helper_Layout('p', array('class' => 'message error'));
    ...
    public function description($description)
    {
        ...
        if (empty($this->description)) {
            $this->description = new Edge_Widget_Helper_Layout('p', array('class' => 'description'));
    ...
    {
        $item = new Edge_Widget_Helper_Layout('span');
        $this->multiline[] = $item;
        return $item;
    }
     ...
     *
     * @access public
     * @param Edge_Widget_Helper_Layout $container 容器对象
