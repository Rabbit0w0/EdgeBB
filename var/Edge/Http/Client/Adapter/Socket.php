 ...
 * @package Http
 */
class Edge_Http_Client_Adapter_Socket extends Edge_Http_Client_Adapter
{
    /**
     * 判断适配器是否可用
     *
    ...
    public function httpSend($url)
    {
        ...
        /** 发送POST信息 */
        if (Edge_Http_Client::METHOD_POST == $this->method) {
            if (empty($this->files)) {
        ...
        if (false === $socket) {
            throw new Edge_Http_Client_Exception($errno . ':' . $errstr, 500);
        }
        ...
        while (!feof($socket)) {
            ...
            if (false === $buf || '' === $buf) {
                ...
                //超时判断
                if ($info['timed_out']) {
                    throw new Edge_Http_Client_Exception(__CLASS__ . ': timeout reading from ' . $this->host . ':' . $this->port, 500);
                } else {
                    throw new Edge_Http_Client_Exception(__CLASS__ . ': could not read from ' . $this->host . ':' . $this->port, 500);
                }
