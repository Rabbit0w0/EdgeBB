...
define('__EDGE_MB_SUPPORTED__', function_exists('mb_get_info') && function_exists('mb_regex_encoding'));
...
function _t($string) {
    if (func_num_args() <= 1) {
        return Edge_I18n::translate($string);
    } else {
        $args = func_get_args();
        array_shift($args);
        return vsprintf(Edge_I18n::translate($string), $args);
    }
}
...
function _n($single, $plural, $number) {
    return str_replace('%d', $number, Edge_I18n::ngettext($single, $plural, $number));
}
...
class Edge_Common
{
    ...
    {
        $safePath = rtrim(__EDGE_ROOT_DIR__, '/');
    ...
    {
        /** 设置自动载入函数 */
        spl_autoload_register(array('Edge_Common', '__autoLoad'));

        /** 设置异常截获函数 */
        set_exception_handler(array('Edge_Common', 'exceptionHandle'));
    }
    ...
    {
        if (defined('__EDGE_DEBUG__') && __EDGE_DEBUG__) {
            echo '<pre><code>';
    ...
    public static function error($exception)
    {
        ...
        if ($isException && $exception instanceof Edge_Db_Exception) {
            $code = 500;
            @error_log($message);
            ...
            if ($exception instanceof Edge_Db_Adapter_Exception) {
                $code = 503;
                $message = 'Error establishing a database connection';
            } else if ($exception instanceof Edge_Db_Query_Exception) {
        ...
        if (is_numeric($code) && $code > 200) {
            Edge_Response::setStatus($code);
        }

        $message = nl2br($message);

        if (defined('__EDGE_EXCEPTION_FILE__')) {
            require_once __EDGE_EXCEPTION_FILE__;
        } else {
            echo
<<<EOF
<!DOCTYPE html>
...
EOF;
     ...
     * $banana = Edge_Common::arrayFlatten($fruit, 'banana');
     * print_r($banana);
     ...
     * echo Edge_Common::fixHtml($input);
     ...
     * $output = Edge_Common::stripTags($input, <a href="">);
     * echo $output;
    ...
    public static function stripTags($html, $allowableTags = NULL)
    {
        ...
        $html = preg_replace_callback("/<([_a-z0-9-]+)(\s+[^>]+)?>/is",
            array('Edge_Common', '__filterAttrs'), $html);
    ...
    public static function safeUrl($url)
    {
        ...
        /** 过滤解析串 */
        $params = array_map(array('Edge_Common', '__removeUrlXss'), $params);
    ...
    public static function strToUpper($str)
    {
        if (__EDGE_MB_SUPPORTED__) {
            ...
            return 'UTF-8' == strtoupper(self::$charset)
                ? preg_replace_callback("/[a-z]+/u", array('Edge_Common', '__strToUpper'), $str) : strtoupper($str);
        }
    }
    ...
    {
        if (is_array($str)) {
            return array_map(array('Edge_Common', 'checkStrEncoding'), $str);
        }
    ...
    {
        if (defined('__EDGE_GRAVATAR_PREFIX__')) {
            $url = __EDGE_GRAVATAR_PREFIX__;
        } else {
