...
/**
 * 包含获取数据支持方法的类.
 * 必须定义__EDGE_DB_HOST__, __EDGE_DB_PORT__, __EDGE_DB_NAME__,
 * __EDGE_DB_USER__, __EDGE_DB_PASS__, __EDGE_DB_CHAR__
 *
 * @package Db
 */
class Edge_Db
{
     ...
     */
    public function __construct($adapterName, $prefix = 'edge_')
    {
        /** 获取适配器名称 */
        $this->_adapterName = $adapterName;

        /** 数据库适配器 */
        $adapterName = 'Edge_Db_Adapter_' . $adapterName;

        if (!call_user_func(array($adapterName, 'isAvailable'))) {
            throw new Edge_Db_Exception("Adapter {$adapterName} is not available");
        }
    ...
    public function selectDb($op)
    {
        if (!isset($this->_connectedPool[$op])) {
            if (empty($this->_pool[$op])) {
                /** Edge_Db_Exception */
                throw new Edge_Db_Exception('Missing Database Connection');
            }
    ...
    public function sql()
    {
        return new Edge_Db_Query($this->_adapter, $this->_prefix);
    }
     ...
     *
     * @access public
     * @param Edge_Db $db 数据库实例
     * @param integer $op 数据库操作
     * @return void
     */
    public function addServer($config, $op)
    {
        $this->_config[] = Edge_Config::factory($config);
     ...
     * @return void
     */
    public static function set(Edge_Db $db)
    {
        self::$_instance = $db;
    }
    ...
    public static function get()
    {
        if (empty(self::$_instance)) {
            /** Edge_Db_Exception */
            throw new Edge_Db_Exception('Missing Database Object');
        }
    ...
    public function query($query, $op = self::READ, $action = self::SELECT)
    {
        ...
        /** 在适配器中执行查询 */
        if ($query instanceof Edge_Db_Query) {
        ...
        /** 提交查询 */
        $resource = $this->_adapter->query($query instanceof Edge_Db_Query ?
