...
abstract class Edge_Widget
{
    ...
    public function __construct($request, $response, $params = NULL)
    {
        ...
        $this->parameter = new Edge_Config();
    ...
    public function on($condition)
    {
        if ($condition) {
            ...
            return new Edge_Widget_Helper_Empty();
        }
    }
    ...
    {
        return Edge_Plugin::factory(empty($handle) ? get_class($this) : $handle);
    }
    ...
    public static function widget($alias, $params = NULL, $request = NULL, $enableResponse = true)
    {
        ...
        if (!isset(self::$_widgetPool[$alias])) {
            ...
            if (!class_exists($className)) {
                throw new Edge_Widget_Exception($className);
            }

            /** 初始化request */
            if (!empty($request)) {
                $requestObject = new Edge_Request();
                $requestObject->setParams($request);
            } else {
                $requestObject = Edge_Request::getInstance();
            }

            /** 初始化response */
            $responseObject = $enableResponse ? Edge_Response::getInstance()
            : Edge_Widget_Helper_Empty::getInstance();
