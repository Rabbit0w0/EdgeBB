...
class Edge_Router
{
        ...
        foreach (self::$_routingTable as $key => $route) {
            if (preg_match($route['regx'], $pathInfo, $matches)) {
                ...
                try {
                    ...
                    $widget = Edge_Widget::widget($route['widget'], $parameter, $params);
    ...
    public static function dispatch()
    {
        ...
        foreach (self::$_routingTable as $key => $route) {
            if (preg_match($route['regx'], $pathInfo, $matches)) {
                ...
                try {
                    ...
                    $widget = Edge_Widget::widget($route['widget'], NULL, $params);

                    if (isset($route['action'])) {
                        $widget->{$route['action']}();
                    }

                    Edge_Response::callback();
                    return;
        ...
        /** 载入路由异常支持 */
        throw new Edge_Router_Exception("Path '{$pathInfo}' not found", 404);
    }
    ...
    public static function url($name, array $value = NULL, $prefix = NULL)
    {
        ...
        return Edge_Common::url(vsprintf($route['format'], $pattern), $prefix);
    }
    ...
    public static function setRoutes($routes)
    {
        if (isset($routes[0])) {
            ...
            /** 解析路由配置 */
            $parser = new Edge_Router_Parser($routes);
