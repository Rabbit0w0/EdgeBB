...
class Widget_Options_Reading extends Widget_Options_Permalink
{
    ...
    {
        /** 构建表格 */
        $form = new Edge_Widget_Helper_Form($this->security->getIndex('/action/options-reading'),
            Edge_Widget_Helper_Form::POST_METHOD);

        /** 文章日期格式 */
        $postDateFormat = new Edge_Widget_Helper_Form_Element_Text('postDateFormat', NULL, $this->options->postDateFormat,
...
<label for="frontArchive">' . _t('同时将文章列表页路径更改为 %s',
        ...
        $frontPage = new Edge_Widget_Helper_Form_Element_Radio('frontPage', $frontPageOptions,
        $frontPageType, _t('站点首页'));
        $form->addInput($frontPage->multiMode());

        /** 文章列表数目 */
        $postsListSize = new Edge_Widget_Helper_Form_Element_Text('postsListSize', NULL, $this->options->postsListSize,
        ...
        /** 每页文章数目 */
        $pageSize = new Edge_Widget_Helper_Form_Element_Text('pageSize', NULL, $this->options->pageSize,
        ...
        /** FEED全文输出 */
        $feedFullText = new Edge_Widget_Helper_Form_Element_Radio('feedFullText', array('0' => _t('仅输出摘要'), '1' => _t('全文输出')),
    ...
    public function updateReadingSettings()
    {
        ...
        file_exists(__EDGE_ROOT_DIR__ . '/' . __EDGE_THEME_DIR__ . '/' . $this->options->theme . '/' .
