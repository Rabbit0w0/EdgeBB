...
class Widget_Options_General extends Widget_Abstract_Options implements Widget_Interface_Do
{
    ...
    public static function getLangs()
    {
        ...
        if (!empty($files)) {
            foreach ($files as $file) {
                $getText = new Edge_I18n_GetText($file, false);
    ...
    {
        /** 构建表格 */
        $form = new Edge_Widget_Helper_Form($this->security->getIndex('/action/options-general'),
        Edge_Widget_Helper_Form::POST_METHOD);

        /** 站点名称 */
        $title = new Edge_Widget_Helper_Form_Element_Text('title', NULL, $this->options->title, _t('站点名称'), _t('站点的名称将显示在网页的标题处.'));
        ...
        if (!defined('__EDGE_SITE_URL__')) {
            $siteUrl = new Edge_Widget_Helper_Form_Element_Text('siteUrl', NULL, $this->options->originalSiteUrl, _t('站点地址'), _t('站点地址主要用于生成内容的永久链接.') . ($this->options->originalSiteUrl == $this->options->rootUrl ? 
        ...
        /** 站点描述 */
        $description = new Edge_Widget_Helper_Form_Element_Text('description', NULL, $this->options->description, _t('站点描述'), _t('站点描述将显示在网页代码的头部.'));
        $form->addInput($description->addRule('xssCheck', _t('请不要在站点描述中使用特殊字符')));

        /** 关键词 */
        $keywords = new Edge_Widget_Helper_Form_Element_Text('keywords', NULL, $this->options->keywords, _t('关键词'), _t('请以半角逗号 "," 分割多个关键字.'));
        $form->addInput($keywords->addRule('xssCheck', _t('请不要在关键词中使用特殊字符')));

        /** 注册 */
        $allowRegister = new Edge_Widget_Helper_Form_Element_Radio('allowRegister', array('0' => _t('不允许'), '1' => _t('允许')), $this->options->allowRegister, _t('是否允许注册'),
        _t('允许访问者注册到你的网站, 默认的注册用户不享有任何写入权限.'));
        $form->addInput($allowRegister);
 
        /** XMLRPC */
        $allowXmlRpc = new Edge_Widget_Helper_Form_Element_Radio('allowXmlRpc', array('0' => _t('关闭'), '1' => _t('仅关闭 Pingback 接口'), '2' => _t('打开')), $this->options->allowXmlRpc, _t('XMLRPC 接口'));
        ...
        if (count($langs) > 1) {
            $lang = new Edge_Widget_Helper_Form_Element_Select('lang', $langs, $this->options->lang, _t('语言'));
        ...
        $timezone = new Edge_Widget_Helper_Form_Element_Select('timezone', $timezoneList, $this->options->timezone, _t('时区'));
        ...
        $attachmentTypes = new Edge_Widget_Helper_Form_Element_Checkbox('attachmentTypes', $attachmentTypesOptions,
