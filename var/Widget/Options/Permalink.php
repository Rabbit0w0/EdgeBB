...
class Widget_Options_Permalink extends Widget_Abstract_Options implements Widget_Interface_Do
{
    ...
    public function checkRule($value)
    {
        ...
        $parser = new Edge_Router_Parser($currentTable);
    ...
    public function checkRewrite($value)
    {
        if ($value) {
            ...
            if (!file_exists(__EDGE_ROOT_DIR__ . '/.htaccess')
                && !Edge_Common::isAppEngine()) {
                if (is_writeable(__EDGE_ROOT_DIR__)) {
                    $parsed = parse_url($this->options->siteUrl);
                    $basePath = empty($parsed['path']) ? '/' : $parsed['path'];
                    $basePath = rtrim($basePath, '/') . '/';

                    $hasWrote = file_put_contents(__EDGE_ROOT_DIR__ . '/.htaccess', "<IfModule mod_rewrite.c>
RewriteEngine On
...
RewriteRule ^(.*)$ {$basePath}index.php/$1 [L]
            ...
            try {
                if ($client) {
                    ...
                    ->send(Edge_Common::url('/action/ajax', $this->options->siteUrl));
                ...
                if (false !== $hasWrote) {
                    @unlink(__EDGE_ROOT_DIR__ . '/.htaccess');
...
RewriteRule . {$basePath}index.php [L]
            ...
            } catch (Edge_Http_Client_Exception $e) {
                if (false !== $hasWrote) {
                    @unlink(__EDGE_ROOT_DIR__ . '/.htaccess');
                }
                return false;
            ...
            return false;
        } else if (file_exists(__EDGE_ROOT_DIR__ . '/.htaccess')) {
            @unlink(__EDGE_ROOT_DIR__ . '/.htaccess');
        }
    ...
    {
        /** 构建表格 */
        $form = new Edge_Widget_Helper_Form($this->security->getRootUrl('index.php/action/options-permalink'),
        Edge_Widget_Helper_Form::POST_METHOD);

        if (!defined('__EDGE_REWRITE__')) {
            /** 是否使用地址重写功能 */
            $rewrite = new Edge_Widget_Helper_Form_Element_Radio('rewrite', array('0' => _t('不启用'), '1' => _t('启用')),
            ...
            if (!$this->options->rewrite && !$this->request->is('enableRewriteAnyway=1')) {
                ...
                if (((isset($_SERVER['SERVER_SOFTWARE']) && false !== strpos(strtolower($_SERVER['SERVER_SOFTWARE']), 'apache'))
                    || function_exists('apache_get_version')) && !file_exists(__EDGE_ROOT_DIR__ . '/.htaccess')
                    && !is_writeable(__EDGE_ROOT_DIR__)) {
        ...
        if (isset($this->request->__edge_form_item_postPattern)) {
            $customPatternValue = $this->request->__edge_form_item_postPattern;
            Edge_Cookie::delete('__edge_form_item_postPattern');
        ...
        $postPattern = new Edge_Widget_Helper_Form_Element_Radio('postPattern', $patterns,
        ...
        /** 独立页面后缀名 */
        $pagePattern = new Edge_Widget_Helper_Form_Element_Text('pagePattern', NULL, $this->decodeRule($this->options->routingTable['page']['url']), _t('独立页面路径'), _t('可用参数: <code>{cid}</code> 页面 ID, <code>{slug}</code> 页面缩略名')
        ...
        /** 分类页面 */
        $categoryPattern = new Edge_Widget_Helper_Form_Element_Text('categoryPattern', NULL, $this->decodeRule($this->options->routingTable['category']['url']), _t('分类路径'), _t('可用参数: <code>{mid}</code> 分类 ID, <code>{slug}</code> 分类缩略名, <code>{directory}</code> 多级分类')
            . '<br />' . _t('请在路径中至少包含上述的一项参数.'));
    ...
    public function updatePermalinkSettings()
    {
        ...
        if ($this->form()->validate()) {
            Edge_Cookie::set('__edge_form_item_postPattern', $this->request->customPattern);
        ...
        $settings = defined('__EDGE_REWRITE__') ? array() : $this->request->from('rewrite');
