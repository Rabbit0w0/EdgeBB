 ...
 * @link edge
 * @package Widget
 * @copyright Copyright (c) 2014 Edge team (http://mcedge.ink)
 * @license MIT License
 */
class Widget_Security extends Edge_Widget
{
    /**
     * @var string
     */
    ...
    public function systemCheck()
    {
        ...
        // 检查安装文件的安全性
        $installFile = __EDGE_ROOT_DIR__ . '/install.php';
        if (file_exists($installFile)) {
            ...
            if (0 !== strpos($installFileContents,
                ...
                false !== strpos($installFileContents,
                    '!isset($_GET[\'finish\']) && file_exists(__EDGE_ROOT_DIR__ . \'/config.inc.php\') && empty($_SESSION[\'edge\'])')) {
                $errors[] = _t('您正在运行一个不安全的安装脚本 <strong>%s</strong>, 请用新版中的对应文件替代或者直接删除它', $installFile);
            }
        }

        // 验证入口文件
        $indexFile = __EDGE_ROOT_DIR__ . '/index.php';
    ...
    {
        return Edge_Common::url($this->getTokenUrl($path), $this->_options->adminUrl);
    }
    ...
    {
        return Edge_Common::url($this->getTokenUrl($path), $this->_options->index);
    }
    ...
    {
        return Edge_Common::url($this->getTokenUrl($path), $this->_options->rootUrl);
    }
