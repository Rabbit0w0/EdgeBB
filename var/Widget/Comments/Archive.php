...
<?php
    ...
    public function execute()
    {
        ...
        $commentsAuthor = Edge_Cookie::get('__edge_remember_author');
        $commentsMail = Edge_Cookie::get('__edge_remember_mail');
    ...
    public function pageNav($prev = '&laquo;', $next = '&raquo;', $splitPage = 3, $splitWord = '...', $template = '')
    {
        if ($this->options->commentsPageBreak && $this->_total > $this->options->commentsPageSize) {
            ...
            $query = Edge_Router::url('comment_page', $pageRow, $this->options->index);

            /** 使用盒状分页 */
            $nav = new Edge_Widget_Helper_PageNavigator_Box($this->_total,
    ...
    {
        //初始化一些变量
        $this->_singleCommentOptions = Edge_Config::factory($singleCommentOptions);
        ...
        if ($this->options->commentsThreaded && !$this->isTopLevel && $this->parameter->allowComment) {
            ...
            if (!$plugged) {
                echo '<a href="' . substr($this->permalink, 0, - strlen($this->theId) - 1) . '?replyTo=' . $this->coid .
                    '#' . $this->parameter->respondId . '" rel="nofollow" onclick="return EdgeComment.reply(\'' .
    ...
    public function cancelReply($word = '')
    {
        if ($this->options->commentsThreaded) {
            ...
            if (!$plugged) {
                ...
                '" rel="nofollow"' . ($replyId ? '' : ' style="display:none"') . ' onclick="return EdgeComment.cancelReply();">' . $word . '</a>';
            }
        }
    }
}
