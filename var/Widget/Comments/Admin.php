...
class Widget_Comments_Admin extends Widget_Abstract_Comments
{
    ...
    public function getMenuTitle()
    {
        ...
        throw new Edge_Widget_Exception(_t('内容不存在'), 404);
    }
    ...
    public function execute()
    {
        ...
        if (!$this->user->pass('editor', true)) {
            ...
            if ('on' == $this->request->__edge_all_comments) {
                Edge_Cookie::set('__edge_all_comments', 'on');
            } else {
                if ('off' == $this->request->__edge_all_comments) {
                    Edge_Cookie::set('__edge_all_comments', 'off');
                }

                if ('on' != Edge_Cookie::get('__edge_all_comments')) {
                    $select->where('table.comments.ownerId = ?', $this->user->uid);
                }
            }
        }
        ...
        $select->order('table.comments.coid', Edge_Db::SORT_DESC)
