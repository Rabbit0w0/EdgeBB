...
class Widget_Users_Edit extends Widget_Abstract_Users implements Widget_Interface_Do
{
    ...
    {
        /** 构建表格 */
        $form = new Edge_Widget_Helper_Form($this->security->getIndex('/action/users-edit'),
        Edge_Widget_Helper_Form::POST_METHOD);

        /** 用户名称 */
        $name = new Edge_Widget_Helper_Form_Element_Text('name', NULL, NULL, _t('用户名 *'), _t('此用户名将作为用户登录时所用的名称.')
        ...
        /** 用户昵称 */
        $screenName = new Edge_Widget_Helper_Form_Element_Text('screenName', NULL, NULL, _t('用户昵称'), _t('用户昵称可以与用户名不同, 用于前台显示.')
        ...
        /** 用户组 */
        $group =  new Edge_Widget_Helper_Form_Element_Select('group', array('subscriber' => _t('关注者'),
                'contributor' => _t('贡献者'), 'editor' => _t('编辑'), 'administrator' => _t('管理员')),
                NULL, _t('用户组'), _t('不同的用户组拥有不同的权限.')
            . '<br />' . _t('具体的权限分配表请<a href="http://docs.mcedge.ink/develop/acl">参考这里</a>.'));
        $form->addInput($group);

        /** 用户动作 */
        $do = new Edge_Widget_Helper_Form_Element_Hidden('do');
        $form->addInput($do);

        /** 用户主键 */
        $uid = new Edge_Widget_Helper_Form_Element_Hidden('uid');
        $form->addInput($uid);

        /** 提交按钮 */
        $submit = new Edge_Widget_Helper_Form_Element_Submit();
    ...
    public function insertUser()
    {
        ...
        /** 转向原页 */
        $this->response->redirect(Edge_Common::url('manage-users.php', $this->options->adminUrl));
    }
    ...
    public function updateUser()
    {
        ...
        /** 转向原页 */
        $this->response->redirect(Edge_Common::url('manage-users.php?' .
