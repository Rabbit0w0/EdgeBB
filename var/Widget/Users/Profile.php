...
class Widget_Users_Profile extends Widget_Users_Edit implements Widget_Interface_Do
{
    ...
    {
        /** 构建表格 */
        $form = new Edge_Widget_Helper_Form($this->security->getIndex('/action/users-profile'),
        Edge_Widget_Helper_Form::POST_METHOD);

        /** 用户昵称 */
        $screenName = new Edge_Widget_Helper_Form_Element_Text('screenName', NULL, NULL, _t('昵称'), _t('用户昵称可以与用户名不同, 用于前台显示.')
            . '<br />' . _t('如果你将此项留空, 将默认使用用户名.'));
        $form->addInput($screenName);

        /** 个人主页地址 */
        $url = new Edge_Widget_Helper_Form_Element_Text('url', NULL, NULL, _t('个人主页地址'), _t('此用户的个人主页地址, 请用 <code>http://</code> 开头.'));
        $form->addInput($url);

        /** 电子邮箱地址 */
        $mail = new Edge_Widget_Helper_Form_Element_Text('mail', NULL, NULL, _t('电子邮箱地址 *'), _t('电子邮箱地址将作为此用户的主要联系方式.')
            . '<br />' . _t('请不要与系统中现有的电子邮箱地址重复.'));
        $form->addInput($mail);

        /** 用户动作 */
        $do = new Edge_Widget_Helper_Form_Element_Hidden('do', NULL, 'profile');
        $form->addInput($do);

        /** 提交按钮 */
        $submit = new Edge_Widget_Helper_Form_Element_Submit('submit', NULL, _t('更新我的档案'));
    ...
    public function optionsForm()
    {
        ...
        /** 撰写设置 */
        $markdown = new Edge_Widget_Helper_Form_Element_Radio('markdown',
        ...
        $xmlrpcMarkdown = new Edge_Widget_Helper_Form_Element_Radio('xmlrpcMarkdown',
        array('0' => _t('关闭'), '1' => _t('打开')),
        ...
        /** 自动保存 */
        $autoSave = new Edge_Widget_Helper_Form_Element_Radio('autoSave',
        array('0' => _t('关闭'), '1' => _t('打开')),
        ...
        $defaultAllow = new Edge_Widget_Helper_Form_Element_Checkbox('defaultAllow',
        array('comment' => _t('可以被评论'), 'ping' => _t('可以被引用'), 'feed' => _t('出现在聚合中')),
        $allow, _t('默认允许'), _t('设置你经常使用的默认允许权限'));
        $form->addInput($defaultAllow);

        /** 用户动作 */
        $do = new Edge_Widget_Helper_Form_Element_Hidden('do', NULL, 'options');
        $form->addInput($do);

        /** 提交按钮 */
        $submit = new Edge_Widget_Helper_Form_Element_Submit('submit', NULL, _t('保存设置'));
        $submit->input->setAttribute('class', 'btn primary');
        $form->addItem($submit);
    ...
    public function personalForm($pluginName, $className, $pluginFileName, &$group)
    {
        ...
        $form->addItem(new Edge_Widget_Helper_Form_Element_Hidden('do', NULL, 'personal'));
        $form->addItem(new Edge_Widget_Helper_Form_Element_Hidden('plugin', NULL, $pluginName));
        $submit = new Edge_Widget_Helper_Form_Element_Submit('submit', NULL, _t('保存设置'));
        $submit->input->setAttribute('class', 'btn primary');
        $form->addItem($submit);
        return $form;
    ...
    public function personalFormList()
    {
        ...
        while ($plugins->next()) {
            if ($plugins->personalConfig) {
                list($pluginFileName, $className) = Edge_Plugin::portal($plugins->name,
    ...
    public function passwordForm()
    {
        ...
        /** 用户密码 */
        $password = new Edge_Widget_Helper_Form_Element_Password('password', NULL, NULL, _t('用户密码'), _t('为此用户分配一个密码.')
            . '<br />' . _t('建议使用特殊字符与字母、数字的混编样式,以增加系统安全性.'));
        $password->input->setAttribute('class', 'w-60');
        $form->addInput($password);

        /** 用户密码确认 */
        $confirm = new Edge_Widget_Helper_Form_Element_Password('confirm', NULL, NULL, _t('用户密码确认'), _t('请确认你的密码, 与上面输入的密码保持一致.'));
        $confirm->input->setAttribute('class', 'w-60');
        $form->addInput($confirm);

        /** 用户动作 */
        $do = new Edge_Widget_Helper_Form_Element_Hidden('do', NULL, 'password');
        $form->addInput($do);

        /** 提交按钮 */
        $submit = new Edge_Widget_Helper_Form_Element_Submit('submit', NULL, _t('更新密码'));
        $submit->input->setAttribute('class', 'btn primary');
        $form->addItem($submit);
    ...
    public function updatePersonal()
    {
        ...
        /** 获取插件入口 */
        list($pluginFileName, $className) = Edge_Plugin::portal($this->request->plugin,
        ...
        if (!$info['personalConfig'] || !isset($activatedPlugins[$pluginName])) {
            throw new Edge_Widget_Exception(_t('无法配置插件'), 500);
        }
        ...
        /** 转向原页 */
        $this->response->redirect(Edge_Common::url('profile.php', $this->options->adminUrl));
    }
