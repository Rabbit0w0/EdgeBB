...
class Widget_Plugins_Edit extends Widget_Abstract_Options implements Widget_Interface_Do
{
    ...
    {
        /** 获取插件入口 */
        list($pluginFileName, $className) = Edge_Plugin::portal($pluginName, $this->options->pluginDir($pluginName));
        $info = Edge_Plugin::parseInfo($pluginFileName);

        /** 检测依赖信息 */
        list ($version, $build) = explode('/', Edge_Common::VERSION);
        if (Edge_Plugin::checkDependence($build, $info['dependence'])) {
            ...
            || !method_exists($className, 'activate')) {
                throw new Edge_Widget_Exception(_t('无法启用插件'), 500);
            }

            try {
                $result = call_user_func(array($className, 'activate'));
                Edge_Plugin::activate($pluginName);
                $this->update(array('value' => serialize(Edge_Plugin::export())),
            ...
            $form = new Edge_Widget_Helper_Form();
            call_user_func(array($className, 'config'), $form);

            $personalForm = new Edge_Widget_Helper_Form();
    ...
    public function config($pluginName)
    {
        ...
        /** 转向原页 */
        $this->response->redirect(Edge_Common::url('plugins.php', $this->options->adminUrl));
    }
