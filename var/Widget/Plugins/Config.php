...
class Widget_Plugins_Config extends Widget_Abstract_Options
{
    ...
    public function execute()
    {
        ...
        if (empty($config)) {
            throw new Edge_Widget_Exception(_t('插件不存在'), 404);
        }

        /** 获取插件入口 */
        list($this->_pluginFileName, $this->_className) = Edge_Plugin::portal($config,
            $this->options->pluginDir($config));
        $this->info = Edge_Plugin::parseInfo($this->_pluginFileName);
    }
    ...
    public function config()
    {
        ...
        $form = new Edge_Widget_Helper_Form($this->security->getIndex('/action/plugins-edit?config=' . $pluginName),
