...
class Widget_Archive extends Widget_Abstract_Contents
{
     ...
     *
     * @access private
     * @var Edge_Widget_Helper_PageNavigator
     */
    private $_pageNav;
    ...
    public function __construct($request, $response, $params = NULL)
    {
        ...
        if (NULL == $this->parameter->type) {
            $this->parameter->type = Edge_Router::$current;
        } else {
        ...
        if ('feed' == $this->parameter->type) {
            ...
            if ('/comments/' == $feedQuery || '/comments' == $feedQuery) {
                ...
                $matched = Edge_Router::match($this->request->feed, 'pageSize=10&isFeed=1');
                if ($matched && $matched instanceof Widget_Archive) {
                    $this->import($matched);
                } else {
                    throw new Edge_Widget_Exception(_t('聚合页不存在'), 404);
                }
            }

            /** 初始化聚合器 */
            $this->setFeed(new Edge_Feed(Edge_Common::VERSION, $this->_feedType, $this->options->charset, _t('zh-CN')));
    ...
    private function checkPermalink()
    {
        ...
        if ($this->_archiveSingle) {
            ...
            $path = Edge_Router::url($type, $value);
            $permalink = Edge_Common::url($path, $this->options->index);
        }
     ...
     *
     * @access private
     * @param Edge_Db_Query $select 查询对象
     * @param boolean $hasPushed 是否已经压入队列
     * @return void
     */
    private function indexHandle(Edge_Db_Query $select, &$hasPushed)
    {
     ...
     * @throws Edge_Widget_Exception
     */
    private function archiveEmptyHandle(Edge_Db_Query $select, &$hasPushed)
    {
        throw new Edge_Widget_Exception(_t('请求的地址不存在'), 404);
    }
     ...
     * @return void
     */
    private function error404Handle(Edge_Db_Query $select, &$hasPushed)
    {
        /** 设置header */
     ...
     * @throws Edge_Widget_Exception
     */
    private function singleHandle(Edge_Db_Query $select, &$hasPushed)
    {
        if ('comment_page' == $this->parameter->type) {
            $params = array();
            $matched = Edge_Router::match($this->request->permalink);
        ...
        if ($this->request->isPost()
            ...
            Edge_Cookie::set('protectPassword_' . $this->request->filter('int')->protectCID, $this->request->protectPassword, 0);
        }
        ...
        if (!$this->_makeSinglePageAsFrontPage) {
            ...
            /** 设置关键词 */
            $this->_keywords = implode(',', Edge_Common::arrayFlatten($this->tags, 'name'));
     ...
     * @throws Edge_Widget_Exception
     */
    private function categoryHandle(Edge_Db_Query $select, &$hasPushed)
    {
        /** 如果是分类 */
        ...
        if (isset($directory) && ($this->request->directory != implode('/', $category['directory']))) {
            throw new Edge_Widget_Exception(_t('父级分类不存在'), 404);
        }
     ...
     * @throws Edge_Widget_Exception
     */
    private function tagHandle(Edge_Db_Query $select, &$hasPushed)
    {
     ...
     * @throws Edge_Widget_Exception
     */
    private function authorHandle(Edge_Db_Query $select, &$hasPushed)
    {
        ...
        if (!$author) {
            throw new Edge_Widget_Exception(_t('作者不存在'), 404);
        }
     ...
     * @return void
     */
    private function dateHandle(Edge_Db_Query $select, &$hasPushed)
    {
        /** 如果是按日期归档 */
        $year = $this->request->filter('int')->year;
        ...
        /** RSS 2.0 */
        $this->_feedUrl = Edge_Router::url($currentRoute, $value, $this->options->feedUrl);

        /** RSS 1.0 */
        $this->_feedRssUrl = Edge_Router::url($currentRoute, $value, $this->options->feedRssUrl);

        /** ATOM 1.0 */
        $this->_feedAtomUrl = Edge_Router::url($currentRoute, $value, $this->options->feedAtomUrl);
     ...
     * @return void
     */
    private function searchHandle(Edge_Db_Query $select, &$hasPushed)
    {
        /** 增加自定义搜索引擎接口 */
        //~ fix issue 40
        ...
        /** 设置头部feed */
        /** RSS 2.0 */
        $this->_feedUrl = Edge_Router::url('search', array('keywords' => $keywords), $this->options->feedUrl);

        /** RSS 1.0 */
        $this->_feedRssUrl = Edge_Router::url('search', array('keywords' => $keywords), $this->options->feedAtomUrl);

        /** ATOM 1.0 */
        $this->_feedAtomUrl = Edge_Router::url('search', array('keywords' => $keywords), $this->options->feedAtomUrl);
    ...
    public function execute()
    {
        ...
        if (isset($this->request->s)) {
            ...
            if (NULL != $filterKeywords) {
                $this->response->redirect(Edge_Router::url('search',
    ...
    public function pageNav($prev = '&laquo;', $next = '&raquo;', $splitPage = 3, $splitWord = '...', $template = '')
    {
        if ($this->have()) {
            ...
            if (!$hasNav && $total > $this->parameter->pageSize) {
                $query = Edge_Router::url($this->parameter->type .
                (false === strpos($this->parameter->type, '_page') ? '_page' : NULL),
                $this->_pageRow, $this->options->index);

                /** 使用盒状分页 */
                $nav = new Edge_Widget_Helper_PageNavigator_Box($total, 
    ...
    public function pageLink($word = '&laquo; Previous Entries', $page = 'prev')
    {
        if ($this->have()) {
            if (empty($this->_pageNav)) {
                ...
                /** 使用盒状分页 */
                $this->_pageNav = new Edge_Widget_Helper_PageNavigator_Classic($this->getTotal(),
                $this->_currentPage, $this->parameter->pageSize, $query);
            }
    ...
    public function theNext($format = '%s', $default = NULL, $custom = array())
    {
        $content = $this->db->fetchRow($this->select()->where('table.contents.created > ? AND table.contents.created < ?',
            ...
            ->where("table.contents.password IS NULL OR table.contents.password = ''")
            ->order('table.contents.created', Edge_Db::SORT_ASC)
            ->limit(1));
...
(function () {
    ...
    document[event.add](event.load, function () {
        ...
        input.type = 'hidden';
        input.name = '_';
        input.value = " . Edge_Common::shuffleScriptVar(
    ...
    public function remember($cookieName, $return = false)
    {
        ...
        $value = Edge_Cookie::get('__edge_remember_' . $cookieName);
        if ($return) {
            return $value;
        } else {
    ...
    public function feed()
    {
        ...
        $this->options->siteUrl : Edge_Common::url($this->request->feed, $this->options->index));
        ...
        if ($this->is('single') || 'comments' == $this->parameter->type) {
            ...
            while ($this->next()) {
                ...
                $feedUrl = '';
                if (Edge_Feed::RSS2 == $this->_feedType) {
                    $feedUrl = $this->feedUrl;
                } else if (Edge_Feed::RSS1 == $this->_feedType) {
                    $feedUrl = $this->feedRssUrl;
                } else if (Edge_Feed::ATOM1 == $this->_feedType) {
