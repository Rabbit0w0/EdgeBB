...
class Widget_Abstract_Metas extends Widget_Abstract
{
    ...
    public function filter(array $value)
    {
        ...
        $value['permalink'] = $routeExists ? Edge_Router::url($type, $value, $this->options->index) : '#';
    ...
    {
        $contents = Edge_Common::arrayFlatten($this->db->fetchAll($this->select('cid')
        ->from('table.relationships')
        ->where('mid = ?', $mid)), 'cid');

        foreach ($metas as $meta) {
            if ($mid != $meta) {
                $existsContents = Edge_Common::arrayFlatten($this->db->fetchAll($this->db
    ...
    {
        // 取出count为0的标签
        $tags = Edge_Common::arrayFlatten($this->db->fetchAll($this->db->select('mid')
