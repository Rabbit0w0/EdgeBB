...
class Widget_Abstract_Contents extends Widget_Abstract
{
    ...
    protected function ___description()
    {
        ...
        return Edge_Common::subStr($plainTxt, 0, 100, '...');
    }
    ...
    protected function ___excerpt()
    {
        ...
        return Edge_Common::fixHtml($this->pluginHandle(__CLASS__)->excerptEx($excerpt, $this));
    }
    ...
    protected function ___commentUrl()
    {
        /** 生成反馈地址 */
        /** 评论 */
        return Edge_Router::url('feedback',
     ...
     * @return integer
     */
    public function update(array $content, Edge_Db_Query $condition)
    {
        /** 首先验证写入权限 */
    ...
    {
        $exists = array_flip(Edge_Common::arrayFlatten($this->db->fetchAll($this->db->select('name')
     ...
     *
     * @access public
     * @param Edge_Db_Query $condition 条件
     * @return mixed
     */
    public function isWriteable(Edge_Db_Query $condition)
    {
    ...
    public function getTemplates()
    {
        ...
        foreach ($files as $file) {
            $info = Edge_Plugin::parseInfo($file);
            $file = basename($file);
    ...
    public function filter(array $value)
    {
        ...
        $value['date'] = new Edge_Date($value['created']);
        ...
        $type = $value['type'];
        $routeExists = (NULL != Edge_Router::get($type));
        ...
        /** 生成静态路径 */
        $value['pathinfo'] = $routeExists ? Edge_Router::url($type, $value) : '#';

        /** 生成静态链接 */
        $value['permalink'] = Edge_Common::url($value['pathinfo'], $this->options->index);
        ...
        if ('attachment' == $type) {
            ...
            //增加数据信息
            $value['attachment'] = new Edge_Config($content);
        ...
        /** 生成聚合链接 */
        /** RSS 2.0 */
        $value['feedUrl'] = $routeExists ? Edge_Router::url($type, $value, $this->options->feedUrl) : '#';

        /** RSS 1.0 */
        $value['feedRssUrl'] = $routeExists ? Edge_Router::url($type, $value, $this->options->feedRssUrl) : '#';

        /** ATOM 1.0 */
        $value['feedAtomUrl'] = $routeExists ? Edge_Router::url($type, $value, $this->options->feedAtomUrl) : '#';
        ...
        $value['password'] !== Edge_Cookie::get('protectPassword_' . $value['cid']) &&
        ...
        !$this->user->pass('editor', true)) {
            ...
            if ($this->request->isPost() && isset($this->request->protectPassword)) {
                throw new Edge_Widget_Exception(_t('对不起,您输入的密码错误'), 403);
            }
        }
    ...
    {
        echo Edge_Common::subStr(strip_tags($this->excerpt), 0, $length, $trim);
    }
    ...
    public function title($length = 0, $trim = '...')
    {
        ...
        if (!$plugged) {
            echo $length > 0 ? Edge_Common::subStr($this->title, 0, $length, $trim) : $this->title;
        } else {
            echo $title;
        }
    }
