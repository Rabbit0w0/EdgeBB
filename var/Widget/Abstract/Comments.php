...
class Widget_Abstract_Comments extends Widget_Abstract
{
    ...
    protected function ___permalink()
    {
        ...
        if ($this->options->commentsPageBreak && 'approved' == $this->status) {
            ...
            return Edge_Router::url('comment_page',
    ...
    protected function ___content()
    {
        ...
        return Edge_Common::stripTags($text, '<p><br>' . $this->options->commentsHTMLTagAllowed);
    }
    ...
    public function insert(array $comment)
    {
        ...
        /** 过长的客户端字符串要截断 */
        if (Edge_Common::strLen($insertStruct['agent']) > 511) {
            $insertStruct['agent'] = Edge_Common::subStr($insertStruct['agent'], 0, 511, '');
        }
     ...
     * @return integer
     */
    public function update(array $comment, Edge_Db_Query $condition)
    {
        /** 获取内容主键 */
     ...
     * @return mixed
     */
    public function commentIsWriteable(Edge_Db_Query $condition = NULL)
    {
        if (empty($condition)) {
    ...
    public function gravatar($size = 32, $default = NULL)
    {
        if ($this->options->commentsAvatar && 'comment' == $this->type) {
            ...
            if (!$plugged) {
                $url = Edge_Common::gravatarUrl($this->mail, $size, $rating, $default, $this->request->isSecure());
    ...
    {
        echo Edge_Common::subStr(strip_tags($this->content), 0, $length, $trim);
    }
