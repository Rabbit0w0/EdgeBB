...
class Widget_Abstract_Users extends Widget_Abstract
{
    ...
    protected function ___options()
    {
        ...
        return new Edge_Config($options);
    }
    ...
    {
        //生成静态链接
        $routeExists = (NULL != Edge_Router::get('author'));

        $value['permalink'] = $routeExists ? Edge_Router::url('author', $value, $this->options->index) : '#';

        /** 生成聚合链接 */
        /** RSS 2.0 */
        $value['feedUrl'] = $routeExists ? Edge_Router::url('author', $value, $this->options->feedUrl) : '#';

        /** RSS 1.0 */
        $value['feedRssUrl'] = $routeExists ? Edge_Router::url('author', $value, $this->options->feedRssUrl) : '#';

        /** ATOM 1.0 */
        $value['feedAtomUrl'] = $routeExists ? Edge_Router::url('author', $value, $this->options->feedAtomUrl) : '#';
     ...
     * @return integer
     */
    public function update(array $rows, Edge_Db_Query $condition)
    {
