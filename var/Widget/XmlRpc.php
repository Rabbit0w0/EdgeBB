...
class Widget_XmlRpc extends Widget_Abstract_Contents implements Widget_Interface_Do
{
    ...
    private function getPostExtended(Widget_Abstract_Contents $content)
    {
        ...
        return array(
            $this->options->xmlrpcMarkdown? $post[0] : Edge_Common::fixHtml($post[0]),
            isset($post[1]) ? Edge_Common::fixHtml($post[1]) : NULL
        );
    }
    
    /**
     * 将edge的状态类型转换为wordperss的风格
     * 
     * @access private
     * @param string $status edge的状态
     * @param string $type 内容类型
     * @return string
     */
    private function edgeToWordpressStatus($status, $type = 'post')
    {
        if ('post' == $type) {
            /** 文章状态 */
            switch ($status) {
     ...
     * @return string
     */
    private function wordpressToEdgeStatus($status, $type = 'post')
    {
        if ('post' == $type) {
            /** 文章状态 */
            switch ($status) {
    ...
    public function wpGetPage($blogId, $pageId, $userName, $password)
    {
        ...
        $pageStruct = array(
            ...
            'page_id'       => $page->cid,
            'page_status'   => $this->edgeToWordpressStatus($page->status, 'page'),
            'description'   => $excerpt,
            'title'         => $page->title,
            'link'          => $page->permalink,
    ...
    public function wpGetPages($blogId, $userName, $password)
    {
        ...
        while ($pages->next()) {
            ...
            $pageStructs[] = array(
                ...
                'page_status'            => $this->edgeToWordpressStatus(($pages->hasSaved || 'page_draft' == $pages->type) ? 'draft' : $pages->status, 'page'),
                'description'            => $excerpt,
                'title'                  => $pages->title,
    ...
    public function wpNewCategory($blogId, $userName, $password, $category)
    {
        ...
        $input['slug'] = Edge_Common::slugName(empty($category['slug']) ? $category['name'] : $category['slug']);
    ...
    public function wpSuggestCategories($blogId, $userName, $password, $category, $max_results)
    {
        ...
        /** 构造出查询语句并且查询*/
        $key = Edge_Common::filterSearchQuery($category);
        $key = '%' . $key . '%';
    ...
    public function wpGetComment($blogId, $userName, $password, $commentId)
    {
        ...
        return array(
			...
			'status'				=> $this->edgeToWordpressStatus($comment->status, 'comment'),
    ...
    public function wpGetComments($blogId, $userName, $password, $struct)
    {
        ...
        if (!empty($struct['status'])) {
            $input['status'] = $this->wordpressToEdgeStatus($struct['status'], 'comment');
        } else {
            $input['__edge_all_comments'] = 'on';
        }
        ...
        while ($comments->next()) {
            $commentsStruct[] = array(
                ...
                'status'				=> $this->edgeToWordpressStatus($comments->status, 'comment'),
    ...
    public function wpNewComment($blogId, $userName, $password, $path, $struct)
    {
        ...
        if (is_numeric($path)) {
            ...
            /** 检查目标地址是否正确*/
            $pathInfo = Edge_Common::url(substr($path, strlen($this->options->index)), '/');
            $post = Edge_Router::match($pathInfo);
        }
    ...
    public function mwNewPost($blogId, $userName, $password, $content, $publish)
    {
        ...
        if (isset($content["{$type}_status"])) {
            $status = $this->wordpressToEdgeStatus($content["{$type}_status"], $type);
    ...
    public function mwGetPost($postId, $userName, $password)
    {
        ...
        /** 只需要分类的name*/
        $categories = Edge_Common::arrayFlatten($post->categories, 'name');
        $tags = Edge_Common::arrayFlatten($post->tags, 'name');
        ...
        $postStruct = array(
                ...
                'post_status'   => $this->edgeToWordpressStatus($post->status, 'post'),
                'custom_fields' => array(),
                'sticky'        => 0
        );
    ...
    public function mwGetRecentPosts($blogId, $userName, $password, $postsNum)
    {
        ...
        while ($posts->next()) {
            ...
            /** 只需要分类的name*/
            /** 可以用flatten函数处理 */
            $categories = Edge_Common::arrayFlatten($posts->categories, 'name');
            $tags = Edge_Common::arrayFlatten($posts->tags, 'name');
            ...
            $postStructs[] = array(
                    ...
                    'post_status'            => $this->edgeToWordpressStatus(($posts->hasSaved || 'post_draft' == $posts->type) ? 'draft' : $posts->status, 'post'),
                    'custom_fields'          => array(),
    ...
    public function mtSetPostCategories($postId, $userName, $password, $categories)
    {
        ...
        $post->setCategories($postId, Edge_Common::arrayFlatten($categories, 'categoryId'),
        'publish' == $post->status);
        return true;
    }
    ...
    {
        /** 检查目标地址是否正确*/
        $pathInfo = Edge_Common::url(substr($target, strlen($this->options->index)), '/');
        $post = Edge_Router::match($pathInfo);
        ...
        if (!Edge_Common::checkSafeHost($params['host'])) {
            return new IXR_Error(16, _t('源地址服务器错误'));
        }
        ...
        if ($post) {
            ...
            if ($post->allowPing) {
                ...
                if ($pingNum <= 0) {
                    ...
                    $finalTitle = Edge_Common::removeXSS(trim(strip_tags($matchTitle[1])));

                    /** 干掉html tag，只留下<a>*/
                    $text = Edge_Common::stripTags($response, '<a href="">');
                        ...
                        if (NULL != $line) {
                            if (preg_match("|<a[^>]*href=[\"']{$pregLink}[\"'][^>]*>(.*?)</a>|",$line)) {
                                if (strlen($line) > strlen($finalText)) {
                                    /** <a>也要干掉，*/
                                    $finalText = Edge_Common::stripTags($line);
                                }
                            }
                        }
                    }
                    ...
                    $finalText = '[...]' . Edge_Common::subStr($finalText, 0, 200, '') . '[...]';
                    ...
                    $pingback = array(
                        ...
                        'author'    =>  Edge_Common::subStr($finalTitle, 0, 150, '...'),
                        'url'       =>  Edge_Common::safeUrl($source),
                        'text'      =>  $finalText,
...
<rsd version="1.0" xmlns="http://archipelago.phrasewise.com/rsd">
    <service>
        <engineName>Edge</engineName>
        <engineLink>http://www.mcedge.ink/</engineLink>
