...
class Widget_Metas_Category_Edit extends Widget_Abstract_Metas implements Widget_Interface_Do
{
    ...
    {
        /** 构建表格 */
        $form = new Edge_Widget_Helper_Form($this->security->getIndex('/action/metas-category-edit'),
            Edge_Widget_Helper_Form::POST_METHOD);

        /** 分类名称 */
        $name = new Edge_Widget_Helper_Form_Element_Text('name', NULL, NULL, _t('分类名称 *'));
        $form->addInput($name);

        /** 分类缩略名 */
        $slug = new Edge_Widget_Helper_Form_Element_Text('slug', NULL, NULL, _t('分类缩略名'),
        ...
        $parent = new Edge_Widget_Helper_Form_Element_Select('parent', $options, $this->request->parent, _t('父级分类'),
        _t('此分类将归档在您选择的父级分类下.'));
        $form->addInput($parent);

        /** 分类描述 */
        $description =  new Edge_Widget_Helper_Form_Element_Textarea('description', NULL, NULL,
        ...
        /** 分类主键 */
        $mid = new Edge_Widget_Helper_Form_Element_Hidden('mid');
        $form->addInput($mid);
        ...
        if (isset($this->request->mid) && 'insert' != $action) {
            ...
            if (!$meta) {
                $this->response->redirect(Edge_Common::url('manage-categories.php', $this->options->adminUrl));
            }
    ...
    public function insertCategory()
    {
        ...
        $category['slug'] = Edge_Common::slugName(empty($category['slug']) ? $category['name'] : $category['slug']);
        ...
        /** 转向原页 */
        $this->response->redirect(Edge_Common::url('manage-categories.php'
