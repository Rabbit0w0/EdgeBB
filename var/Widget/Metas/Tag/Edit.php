...
class Widget_Metas_Tag_Edit extends Widget_Abstract_Metas implements Widget_Interface_Do
{
    ...
    {
        /** 构建表格 */
        $form = new Edge_Widget_Helper_Form($this->security->getIndex('/action/metas-tag-edit'),
            Edge_Widget_Helper_Form::POST_METHOD);

        /** 标签名称 */
        $name = new Edge_Widget_Helper_Form_Element_Text('name', NULL, NULL,
        _t('标签名称 *'), _t('这是标签在站点中显示的名称.可以使用中文,如 "地球".'));
        $form->addInput($name);

        /** 标签缩略名 */
        $slug = new Edge_Widget_Helper_Form_Element_Text('slug', NULL, NULL,
        ...
        if (isset($this->request->mid) && 'insert' != $action) {
            ...
            if (!$meta) {
                $this->response->redirect(Edge_Common::url('manage-tags.php', $this->options->adminUrl));
            }
    ...
    public function insertTag()
    {
        ...
        $tag['type'] = 'tag';
        $tag['slug'] = Edge_Common::slugName(empty($tag['slug']) ? $tag['name'] : $tag['slug']);
