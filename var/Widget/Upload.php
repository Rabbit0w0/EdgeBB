...
class Widget_Upload extends Widget_Abstract_Contents implements Widget_Interface_Do
{
    ...
    public static function uploadHandle($file)
    {
        ...
        $result = Edge_Plugin::factory('Widget_Upload')->trigger($hasUploaded)->uploadHandle($file);
        if ($hasUploaded) {
            return $result;
        }
        ...
        if (!self::checkFileType($ext) || Edge_Common::isAppEngine()) {
            return false;
        }

        $date = new Edge_Date();
        $path = Edge_Common::url(defined('__EDGE_UPLOAD_DIR__') ? __EDGE_UPLOAD_DIR__ : self::UPLOAD_DIR,
            defined('__EDGE_UPLOAD_ROOT_DIR__') ? __EDGE_UPLOAD_ROOT_DIR__ : __EDGE_ROOT_DIR__) 
        ...
        return array(
            'name' => $file['name'],
            'path' => (defined('__EDGE_UPLOAD_DIR__') ? __EDGE_UPLOAD_DIR__ : self::UPLOAD_DIR) 
                . '/' . $date->year . '/' . $date->month . '/' . $fileName,
            'size' => $file['size'],
            'type' => $ext,
            'mime' => Edge_Common::mimeContentType($path)
        );
    }
    ...
    public static function modifyHandle($content, $file)
    {
        ...
        $result = Edge_Plugin::factory('Widget_Upload')->trigger($hasModified)->modifyHandle($content, $file);
        if ($hasModified) {
            return $result;
        }
        ...
        if ($content['attachment']->type != $ext || Edge_Common::isAppEngine()) {
            return false;
        }

        $path = Edge_Common::url($content['attachment']->path, 
            defined('__EDGE_UPLOAD_ROOT_DIR__') ? __EDGE_UPLOAD_ROOT_DIR__ : __EDGE_ROOT_DIR__);
        $dir = dirname($path);
    ...
    {
        $result = Edge_Plugin::factory('Widget_Upload')->trigger($hasDeleted)->deleteHandle($content);
        if ($hasDeleted) {
            return $result;
        }

        return !Edge_Common::isAppEngine() 
            && @unlink(__EDGE_ROOT_DIR__ . '/' . $content['attachment']->path);
    }
    ...
    {
        $result = Edge_Plugin::factory('Widget_Upload')->trigger($hasPlugged)->attachmentHandle($content);
        if ($hasPlugged) {
            return $result;
        }

        $options = Edge_Widget::widget('Widget_Options');
        return Edge_Common::url($content['attachment']->path, 
            defined('__EDGE_UPLOAD_URL__') ? __EDGE_UPLOAD_URL__ : $options->siteUrl);
    }
    ...
    {
        $result = Edge_Plugin::factory('Widget_Upload')->trigger($hasPlugged)->attachmentDataHandle($content);
        if ($hasPlugged) {
            return $result;
        }

        return file_get_contents(Edge_Common::url($content['attachment']->path, 
            defined('__EDGE_UPLOAD_ROOT_DIR__') ? __EDGE_UPLOAD_ROOT_DIR__ : __EDGE_ROOT_DIR__));
    }
