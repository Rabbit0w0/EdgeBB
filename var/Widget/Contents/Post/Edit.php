...
class Widget_Contents_Post_Edit extends Widget_Abstract_Contents implements Widget_Interface_Do
{
    ...
    protected function getPageOffsetQuery($cid, $status = NULL)
    {
        ...
        'on' == $this->request->__edge_all_posts ? 0 : $this->user->uid);
    }
    ...
    public function execute()
    {
        ...
        if (!empty($this->request->cid) && 'delete' != $this->request->do) {
            ...
            if ('post_draft' == $this->type && $this->parent) {
                $this->response->redirect(Edge_Common::url('write-post.php?cid=' . $this->parent, $this->options->adminUrl));
            }

            if (!$this->have()) {
                throw new Edge_Widget_Exception(_t('文章不存在'), 404);
    ...
    public function getDefaultFieldItems()
    {
        ...
        $layout = new Edge_Widget_Helper_Layout();
        $fields = new Edge_Config();
        ...
        foreach ($items as $item) {
            if ($item instanceof Edge_Widget_Helper_Form_Element) {
                ...
                $div = new Edge_Widget_Helper_Layout('div');
    ...
    public function setTags($cid, $tags, $beforeCount = true, $afterCount = true)
    {
        ...
        $tags = array_filter($tags, array('Edge_Validate', 'xssCheck'));

        /** 取出已有tag */
        $existTags = Edge_Common::arrayFlatten($this->db->fetchAll(
    ...
    public function setCategories($cid, array $categories, $beforeCount = true, $afterCount = true)
    {
        ...
        /** 取出已有category */
        $existCategories = Edge_Common::arrayFlatten($this->db->fetchAll(
        $this->db->select('table.metas.mid')
        ->from('table.metas')
        ->join('table.relationships', 'table.relationships.mid = table.metas.mid')
        ->where('table.relationships.cid = ?', $cid)
    ...
    public function writePost()
    {
        ...
        if ($this->request->is('do=publish')) {
            ...
            /** 页面跳转 */
            $this->response->redirect(Edge_Common::url('manage-posts.php?' . $pageQuery, $this->options->adminUrl));
        } else {
            /** 保存文章 */
            ...
            if ($this->request->isAjax()) {
                ...
                /** 返回原页面 */
                $this->response->redirect(Edge_Common::url('write-post.php?cid=' . $this->cid, $this->options->adminUrl));
            }
        }
    }
