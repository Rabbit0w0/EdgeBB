...
class Widget_Contents_Page_Edit extends Widget_Contents_Post_Edit implements Widget_Interface_Do
{
    ...
    public function execute()
    {
        ...
        if (!empty($this->request->cid) && 'delete' != $this->request->do 
            ...
            if ('page_draft' == $this->status && $this->parent) {
                $this->response->redirect(Edge_Common::url('write-page.php?cid=' . $this->parent, $this->options->adminUrl));
            }

            if (!$this->have()) {
                throw new Edge_Widget_Exception(_t('页面不存在'), 404);
    ...
    public function writePage()
    {
        ...
        if ($this->request->is('do=publish')) { 
            ...
            /** 页面跳转 */
            $this->response->redirect(Edge_Common::url('manage-pages.php?', $this->options->adminUrl));
        } else {
            /** 保存文章 */
            ...
            if ($this->request->isAjax()) {
                $created = new Edge_Date($this->options->time);
                ...
                /** 返回原页面 */
                $this->response->redirect(Edge_Common::url('write-page.php?cid=' . $this->cid, $this->options->adminUrl));
            }
        }
    }
