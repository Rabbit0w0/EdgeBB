...
class Widget_Contents_Attachment_Edit extends Widget_Contents_Post_Edit implements Widget_Interface_Do
{
    ...
    public function execute()
    {
        ...
        if ((isset($this->request->cid) && 'delete' != $this->request->do
         && 'insert' != $this->request->do) || 'update' == $this->request->do) {
            ...
            if (!$this->have()) {
                throw new Edge_Widget_Exception(_t('文件不存在'), 404);
            } else if ($this->have() && !$this->allow('edit')) {
                throw new Edge_Widget_Exception(_t('没有编辑权限'), 403);
            }
        }
    }
    ...
    public function nameToSlug($name)
    {
        if (empty($this->request->slug)) {
            $slug = Edge_Common::slugName($name);
    ...
    public function slugExists($slug)
    {
        ...
        ->where('slug = ?', Edge_Common::slugName($slug))
        ->limit(1);
     ...
     *
     * @access public
     * @return Edge_Widget_Helper_Form_Element
     */
    public function form()
    {
        /** 构建表格 */
        $form = new Edge_Widget_Helper_Form($this->security->getIndex('/action/contents-attachment-edit'),
        Edge_Widget_Helper_Form::POST_METHOD);

        /** 文件名称 */
        $name = new Edge_Widget_Helper_Form_Element_Text('name', NULL, $this->title, _t('标题 *'));
        $form->addInput($name);

        /** 文件缩略名 */
        $slug = new Edge_Widget_Helper_Form_Element_Text('slug', NULL, $this->slug, _t('缩略名'),
        _t('文件缩略名用于创建友好的链接形式,建议使用字母,数字,下划线和横杠.'));
        $form->addInput($slug);

        /** 文件描述 */
        $description =  new Edge_Widget_Helper_Form_Element_Textarea('description', NULL, $this->attachment->description,
        _t('描述'), _t('此文字用于描述文件,在有的主题中它会被显示.'));
        $form->addInput($description);

        /** 分类动作 */
        $do = new Edge_Widget_Helper_Form_Element_Hidden('do', NULL, 'update');
        $form->addInput($do);

        /** 分类主键 */
        $cid = new Edge_Widget_Helper_Form_Element_Hidden('cid', NULL, $this->cid);
        $form->addInput($cid);

        /** 提交按钮 */
        $submit = new Edge_Widget_Helper_Form_Element_Submit(NULL, NULL, _t('提交修改'));
        $submit->input->setAttribute('class', 'btn primary');
        $delete = new Edge_Widget_Helper_Layout('a', array(
    ...
    public function updateAttachment()
    {
        ...
        $input['slug'] = Edge_Common::slugName(empty($input['slug']) ? $input['name'] : $input['slug']);
        ...
        /** 转向原页 */
        $this->response->redirect(Edge_Common::url('manage-medias.php?' .
    ...
    public function deleteAttachment()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            /** 返回原网页 */
            $this->response->redirect(Edge_Common::url('manage-medias.php', $this->options->adminUrl));
        }
    }
    ...
    public function clearAttachment()
    {
        ...
        do {
            $posts = Edge_Common::arrayFlatten($this->db->fetchAll($this->select('cid')
                ->from('table.contents')
