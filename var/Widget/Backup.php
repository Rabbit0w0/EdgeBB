...
class Widget_Backup extends Widget_Abstract_Options implements Widget_Interface_Do
{
    ...
    private function buildBuffer($type, $data)
    {
        ...
        return Edge_Common::buildBackupBuffer($type, $header, $body);
    }
    ...
    private function parseHeader($str, &$version) {
        ...
        if (!preg_match("/%EDGE_BACKUP_[A-Z0-9]{4}%/", $str)) {
            return false;
        }
    ...
    private function extractData($file)
    {
        ...
        while (!feof($fp) && $offset + $headerSize < $fileSize) {
            $data = Edge_Common::extractBackupBuffer($fp, $offset, $version);
    ...
    private function processData($type, $header, $body)
    {
        ...
        if (!empty($table)) {
            ...
            Edge_Plugin::factory(__CLASS__)->import($type, $header, $body);
        }
    }
    ...
    private function reLogin(&$user)
    {
        ...
        Edge_Cookie::set('__edge_uid', $user['uid']);
        Edge_Cookie::set('__edge_authCode', Edge_Common::hash($user['authCode']));
        $this->_login = true;
    }
    ...
    private function export()
    {
        ...
        Edge_Plugin::factory(__CLASS__)->export();
        echo $header;
        ob_end_flush();
    }
    ...
    private function import()
    {
        ...
        if (!empty($_FILES)) {
            ...
            $path = __EDGE_BACKUP_DIR__ . '/' . $this->request->get('file');
    ...
    {
        return array_map('basename', glob(__EDGE_BACKUP_DIR__ . '/*.dat'));
    }
