 ...
 * @package Widget
 */
class Widget_Menu extends Edge_Widget
{
    /**
     * 父菜单列表
     *
    ...
    public function execute()
    {
        ...
        foreach ($parentNodes as $key => $parentNode) {
            ...
            foreach ($childNodes[$key] as $inKey => $childNode) {
                ...
                // parse url
                $url = Edge_Common::url($url, $adminUrl);
                ...
                if (!$hidden) {
                    ...
                    if (is_array($name)) {
                        ...
                        $name = Edge_Widget::widget($widget)->$method();
                    }
                    
                    if (is_array($title)) {
                        list($widget, $method) = $title;
                        $title = Edge_Widget::widget($widget)->$method();
                    }

                    if (is_array($addLink)) {
                        list($widget, $method) = $addLink;
                        $addLink = Edge_Widget::widget($widget)->$method();
                    }
                }
                ...
                if ($validate) {
                    ...
                    $this->title = $title;
                    $this->addLink = $addLink ? Edge_Common::url($addLink, $adminUrl) : NULL;
                } 
