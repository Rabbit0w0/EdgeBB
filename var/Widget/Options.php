 ...
 * @license MIT License
 */
class Widget_Options extends Edge_Widget
{
    /**
     * 缓存的插件配置
     *
    ...
    {
        return Edge_Router::url('feed', array('feed' => '/'), $this->index);
    }
    ...
    {
        return Edge_Router::url('feed', array('feed' => '/rss/'), $this->index);
    }
    ...
    {
        return Edge_Router::url('feed', array('feed' => '/atom/'), $this->index);
    }
    ...
    {
        return Edge_Router::url('feed', array('feed' => '/comments/'), $this->index);
    }
    ...
    {
        return Edge_Router::url('feed', array('feed' => '/rss/comments/'), $this->index);
    }
    ...
    {
        return Edge_Router::url('feed', array('feed' => '/atom/comments/'), $this->index);
    }
    ...
    {
        return Edge_Router::url('do', array('action' => 'xmlrpc'), $this->index);
    }
    ...
    {
        return ($this->rewrite || (defined('__EDGE_REWRITE__') && __EDGE_REWRITE__)) 
            ? $this->rootUrl : Edge_Common::url('index.php', $this->rootUrl);
    }
    ...
    {
        return defined('__EDGE_THEME_URL__') ? __EDGE_THEME_URL__ :
            Edge_Common::url(__EDGE_THEME_DIR__ . '/' . $this->theme, $this->siteUrl);
    }
    ...
    {
        return defined('__EDGE_PLUGIN_URL__') ? __EDGE_PLUGIN_URL__ :
            Edge_Common::url(__EDGE_PLUGIN_DIR__, $this->siteUrl);
    }
    ...
    {
        return Edge_Common::url('login.php', $this->adminUrl);
    }
    ...
    protected function ___loginAction()
    {
        return $this->widget('Widget_Security')->getTokenUrl(
            Edge_Router::url('do', array('action' => 'login', 'widget' => 'Login'),
            Edge_Common::url('index.php', $this->rootUrl)));
    }
    ...
    {
        return Edge_Common::url('register.php', $this->adminUrl);
    }
    ...
    {
        return $this->widget('Widget_Security')->getTokenUrl(
            Edge_Router::url('do', array('action' => 'register', 'widget' => 'Register'), $this->index));
    }
    ...
    {
        return Edge_Common::url('profile.php', $this->adminUrl);
    }
    ...
    {
        return $this->widget('Widget_Security')->getTokenUrl(
            Edge_Common::url('/action/logout', $this->index));
    }
    ...
    {
        return Edge_Date::$serverTimezoneOffset;
    }
    ...
    public function execute()
    {
        ...
        /** 初始化站点信息 */
        if (defined('__EDGE_SITE_URL__')) {
            $this->siteUrl = __EDGE_SITE_URL__;
        } else if (defined('__EDGE_DYNAMIC_SITE_URL__') && __EDGE_DYNAMIC_SITE_URL__) {
            $this->siteUrl = $this->rootUrl;
        }

        $this->originalSiteUrl = $this->siteUrl;
        $this->siteUrl = Edge_Common::url(NULL, $this->siteUrl);
        ...
        if (!isset($this->routingTable[0])) {
            /** 解析路由并缓存 */
            $parser = new Edge_Router_Parser($this->routingTable);
    ...
    {
        echo Edge_Common::url($path, $this->siteUrl);
    }
    ...
    {
        echo Edge_Common::url($path, $this->index);
    }
    ...
    {
        if (empty($theme)) {
            echo Edge_Common::url($path, $this->themeUrl);
        }

        $url = defined('__EDGE_THEME_URL__') ? __EDGE_THEME_URL__ :
            Edge_Common::url(__EDGE_THEME_DIR__ . '/' . $theme, $this->siteUrl);

        return Edge_Common::url($path, $url);
    }
    ...
    {
        echo Edge_Common::url($path, $this->pluginUrl);
    }
    ...
    {
        return __EDGE_ROOT_DIR__ . __EDGE_THEME_DIR__ . '/' . trim($theme, './') . '/' . trim($file, './');
    }
    ...
    {
        return __EDGE_ROOT_DIR__ . '/' . __EDGE_PLUGIN_DIR__;
    }
    ...
    {
        echo Edge_Common::url($path, $this->adminUrl);
    }
    ...
    {
        $url = Edge_Common::url($type, $this->adminUrl);

        if (empty($file)) {
            return $url;
        }

        echo Edge_Common::url($file, $url);
    }
    ...
    public function plugin($pluginName)
    {
        if (!isset($this->_pluginConfig[$pluginName])) {
            ...
            && false !== ($options = unserialize($this->row['plugin:' . $pluginName]))) {
                $this->_pluginConfig[$pluginName] = new Edge_Config($options);
            } else {
                throw new Edge_Plugin_Exception(_t('插件%s的配置信息没有找到', $pluginName), 500);
            }
        }
    ...
    public function personalPlugin($pluginName)
    {
        if (!isset($this->_personalPluginConfig[$pluginName])) {
            ...
            && false !== ($options = unserialize($this->row['_plugin:' . $pluginName]))) {
                $this->_personalPluginConfig[$pluginName] = new Edge_Config($options);
            } else {
                throw new Edge_Plugin_Exception(_t('插件%s的配置信息没有找到', $pluginName), 500);
            }
        }
