...
class Widget_Themes_Edit extends Widget_Abstract_Options implements Widget_Interface_Do
{
    ...
    public function changeTheme($theme)
    {
        ...
        if (is_dir($this->options->themeFile($theme))) {
            ...
            throw new Edge_Widget_Exception(_t('您选择的风格不存在'));
        }
    }
    ...
    public function editThemeFile($theme, $file)
    {
        ...
        if (file_exists($path) && is_writeable($path) && !Edge_Common::isAppEngine()
            && (!defined('__EDGE_THEME_WRITEABLE__') || __EDGE_THEME_WRITEABLE__)) {
            ...
            throw new Edge_Widget_Exception(_t('您编辑的文件不存在'));
        }
    }
    ...
    public function config($theme)
    {
        ...
        /** 转向原页 */
        $this->response->redirect(Edge_Common::url('options-theme.php', $this->options->adminUrl));
    }
