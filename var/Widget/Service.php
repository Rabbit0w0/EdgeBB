...
class Widget_Service extends Widget_Abstract_Options implements Widget_Interface_Do
{
    ...
    {
        $url = Edge_Common::url('/action/service', $this->options->index);

        if (defined('__EDGE_SERVICE_URL__')) {
            $rootPath = rtrim(parse_url($this->options->rootUrl, PHP_URL_PATH), '/');
            $path = parse_url($url, PHP_URL_PATH);
            $parts = parse_url(__EDGE_SERVICE_URL__);

            if (!empty($parts['path'])
                && $parts['path'] != '/'
                && rtrim($parts['path'], '/') != $rootPath) {
                $path = Edge_Common::url($path, $parts['path']);
            }

            $parts['path'] = $path;
            $url = Edge_Common::buildUrl($parts);
        }
    ...
    public function sendPingHandle()
    {
        ...
        if (!Edge_Common::timeTokenValidate($token, $this->options->secret, 3)) {
        ...
        if ($post->have() && preg_match_all("|<a[^>]*href=[\"'](.*?)[\"'][^>]*>(.*?)</a>|", $post->text, $matches)) {
            ...
            foreach ($links as $url) {
                ...
                if (isset($urlPart['scheme'])) {
                    ...
                    $urlPart['scheme'] = 'http';
                    $url = Edge_Common::buildUrl($urlPart);
                }
    ...
    public function sendPing($cid, array $trackback = NULL)
    {
        ...
        if ($client = Edge_Http_Client::get()) {
            try {

                $input = array(
                    'do' => 'ping',
                    'cid' => $cid,
                    'token' => Edge_Common::timeToken($this->options->secret)
                );
                ...
                $client->setHeader('User-Agent', $this->options->generator)
                    ->setTimeout(2)
                    ->setData($input)
                    ->setMethod(Edge_Http_Client::METHOD_POST)
    ...
    public function requestService($method, $params = NULL)
    {
        ...
        if (!$called) {
            ...
            Edge_Response::addCallback(function () use ($self) {
                if (!empty($self->asyncRequests) && $client = Edge_Http_Client::get()) {
                    try {
                        $client->setHeader('User-Agent', $this->options->generator)
                            ->setTimeout(2)
                            ->setData(array(
    ...
    public function asyncHandle()
    {
        ...
        $plugin = Edge_Plugin::factory(__CLASS__);
