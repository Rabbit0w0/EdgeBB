...
class Widget_Feedback extends Widget_Abstract_Comments implements Widget_Interface_Do
{
    ...
    private function comment()
    {
        ...
        if ($parentId = $this->request->filter('int')->get('parent')) {
            ...
            ->where('coid = ?', $parentId))) && $this->_content->cid == $parent['cid']) {
                ...
                throw new Edge_Widget_Exception(_t('父级评论不存在'));
            }
        }
        ...
        if (!$this->user->hasLogin()) {
            ...
            Edge_Cookie::set('__edge_remember_author', $comment['author'], $expire);
            Edge_Cookie::set('__edge_remember_mail', $comment['mail'], $expire);
            Edge_Cookie::set('__edge_remember_url', $comment['url'], $expire);
        } else {
        ...
        if ($error = $validator->run($comment)) {
            /** 记录文字 */
            Edge_Cookie::set('__edge_remember_text', $comment['text']);
            throw new Edge_Widget_Exception(implode("\n", $error));
        }
        ...
        Edge_Cookie::delete('__edge_remember_text');
    ...
    private function trackback()
    {
        ...
        ->where('status = ? AND ip = ?', 'spam', $this->request->getIp())) > 0) {
            /** 使用404告诉机器人 */
            throw new Edge_Widget_Exception(_t('找不到内容'), 404);
        }
        ...
        /** 截取长度 */
        $trackback['text'] = Edge_Common::subStr($trackback['text'], 0, 100, '[...]');

        /** 如果库中已经存在重复url则直接拒绝 */
        if ($this->size($this->select()
        ->where('cid = ? AND url = ? AND type <> ?', $this->_content->cid, $trackback['url'], 'comment')) > 0) {
            /** 使用403告诉机器人 */
            throw new Edge_Widget_Exception(_t('禁止重复提交'), 403);
        }
    ...
    public function filterText($text)
    {
        ...
        return Edge_Common::removeXSS(Edge_Common::stripTags(
    ...
    public function action()
    {
        ...
        $this->_content = Edge_Router::match($this->request->permalink);
        ...
        in_array($callback, array('comment', 'trackback'))) {
            ...
            if ('comment' == $callback) {
                ...
                if (!$this->_content->allow('comment')) {
                    throw new Edge_Widget_Exception(_t('对不起,此内容的反馈被禁止.'), 403);
                }
                ...
                if ($this->options->commentsCheckReferer && 'false' != $this->parameter->checkReferer) {
                    ...
                    if (empty($referer)) {
                        throw new Edge_Widget_Exception(_t('评论来源页错误.'), 403);
                    }
                ...
                if (!$this->user->pass('editor', true) && $this->_content->authorId != $this->user->uid &&
                    ...
                    ->order('created', Edge_Db::SORT_DESC)
                    ->limit(1));
                    ...
                    $this->options->time - $latestComment['created'] < $this->options->commentsPostInterval)) {
                        throw new Edge_Widget_Exception(_t('对不起, 您的发言过于频繁, 请稍侯再次发布.'), 403);
                    }
                }
            }
            ...
            if ('trackback' == $callback && !$this->_content->allow('ping')) {
                throw new Edge_Widget_Exception(_t('对不起,此内容的引用被禁止.'), 403);
            }
