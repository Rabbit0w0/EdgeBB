<?php
/**
 * 插件帮手将默认出现在所有的edge发行版中.
 * 因此你可以放心使用它的功能, 以方便你的插件安装在用户的系统里.
 *
 * @package Helper
 * @author qining
 * @version 1.0.0
 * @link http://mcedge.ink
 */
class Helper
{
    /**
    ...
    {
        return Edge_Widget::widget('Widget_Options');
    }
    ...
    {
        return Edge_Widget::widget('Widget_Security');
    }
    ...
    public static function widgetById($table, $pkId)
    {
        ...
        $key = $keys[$table];
        $db = Edge_Db::get();
        $widget = new $className(Edge_Request::getInstance(), Edge_Widget_Helper_Empty::getInstance());
    ...
    {
        Edge_Widget::widget('Widget_Service')->requestService($method, $params);
    }
    ...
    public static function removePlugin($pluginName)
    {
        try {
            /** 获取插件入口 */
            list($pluginFileName, $className) = Edge_Plugin::portal($pluginName, __EDGE_ROOT_DIR__ . '/' . __EDGE_PLUGIN_DIR__);
            ...
            || !method_exists($className, 'deactivate')) {
                throw new Edge_Widget_Exception(_t('无法禁用插件'), 500);
            }
        ...
        try {
            Edge_Plugin::deactivate($pluginName);
            $db->query($db->update('table.options')
            ->rows(array('value' => serialize(Edge_Plugin::export())))
            ->where('name = ?', 'plugins'));
        } catch (Edge_Plugin_Exception $e) {
            //nothing to do
        }
    ...
    public static function addRoute($name, $url, $widget, $action = NULL, $after = NULL)
    {
        ...
        $db = Edge_Db::get();
        return Edge_Widget::widget('Widget_Abstract_Options')->update(array('value' => serialize($routingTable))
    ...
    public static function addAction($actionName, $widgetName)
    {
        ...
        $db = Edge_Db::get();
        return Edge_Widget::widget('Widget_Abstract_Options')->update(array('value' => (self::options()->actionTable = serialize($actionTable)))
    ...
    public static function addMenu($menuName)
    {
        ...
        $db = Edge_Db::get();
        Edge_Widget::widget('Widget_Abstract_Options')->update(array('value' => (self::options()->panelTable = serialize($panelTable)))
    ...
    {
        return Edge_Common::url('extending.php?panel=' . (trim($fileName, '/')), self::options()->adminUrl);
    }
    ...
    public static function replyLink($theId, $coid, $word = 'Reply', $formId = 'respond', $style = 2)
    {
        if (self::options()->commentsThreaded) {
            echo '<a href="#' . $formId . '" rel="nofollow" onclick="return edgeAddCommentReply(\'' .
    ...
    {
        if (self::options()->commentsThreaded) {
            echo '<a href="#' . $formId . '" rel="nofollow" onclick="return edgeCancelCommentReply(\'' .
            $formId . '\');">' . $word . '</a>';
        }
    }
...
var edgeAddCommentReply = function (cid, coid, cfid, style) {
...
var edgeCancelCommentReply = function (cfid) {
    var _cf = document.getElementById(cfid),
