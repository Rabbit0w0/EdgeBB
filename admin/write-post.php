...
<form class="row" role="form" action="<?php $security->index('/action/contents-post-edit'); ?>" method="post" name="write_post">
	...
	<div class="card">
	  <div class="card-body">
		  <h4 class="card-title"><?php include 'page-title.php'; ?></h4>
			...
			<?php $postModifyDate = new Edge_Date($post->draft['modified']); ?>
			...
			<?php $permalink = Edge_Common::url($options->routingTable['post']['url'], $options->index);
			...
			<p class="submit clearfix">
				<span class="right">
					...
					<button type="submit" name="do" value="save" id="btn-save" class="mdui-btn mdui-btn-raised mdui-ripple mdui-color-blue mdui-text-color-white-text"><?php _e('保存草稿'); ?></button>
                            <button type="submit" name="do" value="publish" class="mdui-btn mdui-btn-raised mdui-ripple mdui-color-pink mdui-text-color-white-text" id="btn-submit"><?php _e('发布文章'); ?></button>
					<?php if ($options->markdown && (!$post->have() || $post->isMarkdown)): ?>
					<input type="hidden" name="markdown" value="1" />
					<?php endif; ?>
				</span>
			</p>
			<?php Edge_Plugin::factory('admin/write-post.php')->content($post); ?>
		</div>
	</div>
  </div>
  ...
  <div id="edit-secondary" class="col-md-3 grid-margin stretch-card" role="complementary">
	<div class="card">
	  <div class="card-body">
		<ul class="edge-option-tabs clearfix">
		...
		<div id="tab-advance" class="tab-content">
			<section class="edge-post-option" role="application">
				<label for="date" class="edge-label"><?php _e('发布日期'); ?></label>
				<p><input class="edge-date w-100" type="text" name="date" id="date" value="<?php $post->have() ? $post->date('Y-m-d H:i') : ''; ?>" /></p>
			</section>

			<section class="edge-post-option category-option">
				<label class="edge-label"><?php _e('分类'); ?></label>
				<?php Edge_Widget::widget('Widget_Metas_Category_List')->to($category); ?>
				<ul>
					<?php
					if ($post->have()) {
						$categories = Edge_Common::arrayFlatten($post->categories, 'mid');
					} else {
						$categories = array();
					}
					?>
			...
			<section class="edge-post-option">
				<label for="token-input-tags" class="edge-label"><?php _e('标签'); ?></label>
				<p><input id="tags" name="tags" type="text" value="<?php $post->tags(',', false); ?>" class="w-100 text" /></p>
			</section>

			<?php Edge_Plugin::factory('admin/write-post.php')->option($post); ?>

			<button type="button" id="advance-panel-btn" class="btn btn-xs"><?php _e('高级选项'); ?> <i class="i-caret-down"></i></button>
			<div id="advance-panel">
				<?php if($user->pass('editor', true)): ?>
				<section class="edge-post-option visibility-option">
					<label for="visibility" class="edge-label"><?php _e('公开度'); ?></label>
					<p>
				...
				<section class="edge-post-option allow-option">
					<label class="edge-label"><?php _e('权限控制'); ?></label>
					<ul>
				...
				<section class="edge-post-option">
					<label for="trackback" class="edge-label"><?php _e('引用通告'); ?></label>
					<p><textarea id="trackback" class="w-100 mono" name="trackback" rows="2"></textarea></p>
					<p class="description"><?php _e('每一行一个引用地址, 用回车隔开'); ?></p>
				</section>

				<?php Edge_Plugin::factory('admin/write-post.php')->advanceOption($post); ?>
			</div><!-- end #advance-panel -->

			<?php if($post->have()): ?>
			<?php $modified = new Edge_Date($post->modified); ?>
			<section class="edge-post-option">
				<p class="description">
					<br>&mdash;<br>
					<?php _e('本文由 <a href="%s">%s</a> 撰写',
					Edge_Common::url('manage-posts.php?uid=' . $post->author->uid, $options->adminUrl), $post->author->screenName); ?><br>
...
Edge_Plugin::factory('admin/write-post.php')->trigger($plugged)->richEditor($post);
if (!$plugged) {
    include 'editor-js.php';
}
include 'file-upload-js.php';
include 'custom-fields-js.php';
Edge_Plugin::factory('admin/write-post.php')->bottom($post);
include 'footer.php';
?>
