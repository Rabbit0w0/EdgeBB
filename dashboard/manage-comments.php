...
$comments = Edge_Widget::widget('Widget_Comments_Admin');
$isAllComments = ('on' == $request->get('__edge_all_comments') || 'on' == Edge_Cookie::get('__edge_all_comments'));
?>
		...
		<div class="card">
			...
			<div class="clearfix">
				...
				<?php if($user->pass('editor', true) && !isset($request->cid)): ?>
					<li class="<?php if($isAllComments): ?> current<?php endif; ?>"><a href="<?php echo $request->makeUriByRequest('__edge_all_comments=on'); ?>"><?php _e('所有'); ?></a></li>
					<li class="<?php if(!$isAllComments): ?> current<?php endif; ?>"><a href="<?php echo $request->makeUriByRequest('__edge_all_comments=off'); ?>"><?php _e('我的'); ?></a></li>
				<?php endif; ?>
				</ul>
			...
			<form method="post" name="manage_comments" class="operate-form">
				<div class="table-responsive">
					...
					<?php else: ?>
					<tr>
						<td colspan="4"><h6 class="edge-list-table-title"><em><?php _e('没有评论') ?></em></h6></td>
					</tr>
					<?php endif; ?>
					</tbody>
					</table><!-- end .edge-list-table -->
				</div><!-- end .edge-table-wrap -->
			<?php if(isset($request->cid)): ?>
			<input type="hidden" value="<?php echo htmlspecialchars($request->get('cid')); ?>" name="cid" />
			<?php endif; ?>
...
$(document).ready(function () {
    ...
    function rememberScroll () {
        $(window).bind('beforeunload', function () {
            $.cookie('__edge_comments_scroll', $('body').scrollTop());
        });
    }

    // 自动滚动
    (function () {
        var scroll = $.cookie('__edge_comments_scroll');

        if (scroll) {
            $.cookie('__edge_comments_scroll', null);
