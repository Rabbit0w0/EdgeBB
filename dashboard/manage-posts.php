...
$posts = Edge_Widget::widget('Widget_Contents_Post_Admin');
$isAllPosts = ('on' == $request->get('__edge_all_posts') || 'on' == Edge_Cookie::get('__edge_all_posts'));
?>
...
<div class="row">
	<div class="col-md-12 grid-margin stretch-card">
		<div class="card">
			...
			<div class="clearfix">
				<ul class="edge-option-tabs right">
				<?php if($user->pass('editor', true) && !isset($request->uid)): ?>
					<li class="<?php if($isAllPosts): ?> current<?php endif; ?>"><a href="<?php echo $request->makeUriByRequest('__edge_all_posts=on'); ?>"><?php _e('所有'); ?></a></li>
					<li class="<?php if(!$isAllPosts): ?> current<?php endif; ?>"><a href="<?php echo $request->makeUriByRequest('__edge_all_posts=off'); ?>"><?php _e('我的'); ?></a></li>
				<?php endif; ?>
				</ul>
				<ul class="edge-option-tabs">
			...
			<div class="edge-list-operate clearfix">
				<form method="get">
					<div class="operate">
						<label><i class="sr-only"><?php _e('全选'); ?></i><input type="checkbox" class="edge-table-select-all" /></label>
			...
			</div><!-- end .edge-list-operate -->
			<div class="dropdown-divider"></div>
	
		   <form method="post" name="manage_posts" class="operate-form">
			<div class="table-responsive">
			  <table class="table edge-list-table">
			<thead>
				<tr>
					<th> </th>
					<th> </th>
			...
			<tbody>
				...
				<tr id="<?php $posts->theId(); ?>">
					...
					<td><a href="<?php $options->adminUrl('manage-comments.php?cid=' . ($posts->parentId ? $posts->parentId : $posts->cid)); ?>" class="balloon-button size-<?php echo Edge_Common::splitByCount($posts->commentsNum, 1, 10, 20, 50, 100); ?>" title="<?php $posts->commentsNum(); ?> <?php _e('评论'); ?>"><?php $posts->commentsNum(); ?></a></td>
					<td>
					...
					<?php $modifyDate = new Edge_Date($posts->modified); ?>
				...
				<?php else: ?>
				<tr>
					<td colspan="6"><h6 class="edge-list-table-title"><?php _e('没有任何文章'); ?></h6></td>
				</tr>
				<?php endif; ?>
			</tbody>
				</table>
