...
<?php Edge_Widget::widget('Widget_Plugins_List@activated', 'activated=1')->to($activatedPlugins); ?>
<?php if ($activatedPlugins->have() || !empty($activatedPlugins->activatedPlugins)): ?>
			  ...
			  <table class="table">
				...
				<tbody>
					...
					<tr id="plugin-<?php $activatedPlugins->name(); ?>">
						...
						<img src="<?php $options->adminUrl('images/notice.gif'); ?>" title="<?php _e('%s 无法在此版本的edge下正常工作', $activatedPlugins->title); ?>" alt="<?php _e('%s 无法在此版本的edge下正常工作', $activatedPlugins->title); ?>" class="tiny" />
						<?php endif; ?>
						</td>
	...
	<?php endif; ?>
	<?php Edge_Widget::widget('Widget_Plugins_List@unactivated', 'activated=0')->to($deactivatedPlugins); ?>
			...
			<div class="table-responsive">
			  <table class="table">
				...
				<tbody>
					...
					<?php else: ?>
					<tr>
						<td colspan="5"><h6 class="edge-list-table-title"><?php _e('没有安装插件'); ?></h6></td>
					</tr>
					<?php endif; ?>
				</tbody>
			  </table>
