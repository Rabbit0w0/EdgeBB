				...
				<form method="post" name="manage_users" class="operate-form">
					<div class="table-responsive">
					  <table class="table edge-list-table">
						...
						<tbody>
							<?php Edge_Widget::widget('Widget_Users_Admin')->to($users); ?>
							<?php while($users->next()): ?>
							<tr id="user-<?php $users->uid(); ?>">
								<td><input type="checkbox" value="<?php $users->uid(); ?>" name="uid[]"/></td>
								<td><a href="<?php $options->adminUrl('manage-posts.php?uid=' . $users->uid); ?>" class="balloon-button left size-<?php echo Edge_Common::splitByCount($users->postsNum, 1, 10, 20, 50, 100); ?>"><?php $users->postsNum(); ?></a></td>
