				...
				<form method="post" name="manage_pages" class="operate-form">
					<div class="table-responsive">
					  <table class="table edge-list-table">
						...
						<tbody>
							<?php Edge_Widget::widget('Widget_Contents_Page_Admin')->to($pages); ?>
							<?php if($pages->have()): ?>
							<?php while($pages->next()): ?>
							<tr id="<?php $pages->theId(); ?>">
								<td><input type="checkbox" value="<?php $pages->cid(); ?>" name="cid[]"/></td>
								<td><a href="<?php $options->adminUrl('manage-comments.php?cid=' . $pages->cid); ?>" class="balloon-button size-<?php echo Edge_Common::splitByCount($pages->commentsNum, 1, 10, 20, 50, 100); ?>" title="<?php $pages->commentsNum(); ?> <?php _e('评论'); ?>"><?php $pages->commentsNum(); ?></a></td>
								<td>
								...
								<?php $modifyDate = new Edge_Date($pages->modified); ?>
							...
							<?php else: ?>
							<tr>
								<td colspan="6"><h6 class="edge-list-table-title"><?php _e('没有任何页面'); ?></h6></td>
							</tr>
							<?php endif; ?>
						</tbody>
					 </table>
...
(function () {
    $(document).ready(function () {
        var table = $('.edge-list-table').tableDnD({
            onDrop : function () {
                var ids = [];
