...
include 'menu.php';
include 'TUi.php';
Edge_Widget::widget('Widget_Contents_Page_Edit')->to($page);
?>
  ...
  <div class="col-md-9 grid-margin stretch-card" role="main">
	<div class="card">
	  <div class="card-body">
		...
		<?php $pageModifyDate = new Edge_Date($page->draft['modified']); ?>
		...
		<?php $permalink = Edge_Common::url($options->routingTable['page']['url'], $options->index);
		...
		<?php Edge_Plugin::factory('dashboard/write-page.php')->content($page); ?>
	  </div>
	</div>
  </div>
	  ...
	  <div class="card-body">
		...
		<div id="tab-advance" class="tab-content">
			<section  class="edge-post-option" role="application">
				...
				<p><input class="edge-date w-100" type="text" name="date" id="date" value="<?php $page->have() ? $page->date('Y-m-d H:i') : ''; ?>" /></p>
			</section>

			<section class="edge-post-option">
				<label for="order" class="edge-label"><?php _e('页面顺序'); ?></label>
			...
			<section  class="edge-post-option">
				<label for="template" class="edge-label"><?php _e('自定义模板'); ?></label>
					<p>
			...
			<?php Edge_Plugin::factory('dashboard/write-page.php')->option($page); ?>
			...
			<div id="advance-panel">
				...
				<?php Edge_Plugin::factory('dashboard/write-page.php')->advanceOption($page); ?>
			</div>
			<?php if($page->have()): ?>
			<?php $modified = new Edge_Date($page->modified); ?>
			<section class="edge-post-option">
			<p class="description">
				<br>&mdash;<br>
				<?php _e('本页面由 <a href="%s">%s</a> 创建',
				Edge_Common::url('manage-pages.php?uid=' . $page->author->uid, $options->adminUrl), $page->author->screenName); ?><br>
...
Edge_Plugin::factory('dashboard/write-page.php')->trigger($plugged)->richEditor($page);
if (!$plugged) {
    include 'editor-js.php';
}
include 'file-upload-js.php';
include 'custom-fields-js.php';
Edge_Plugin::factory('dashboard/write-page.php')->bottom($page);
include 'footer.php';
?>
